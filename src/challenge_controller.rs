...
use kube::runtime::{Controller, controller::Action, finalizer::Event};
use std::sync::Arc;
...
async fn reconcile(
    ...
    context: Arc<ChallengerControllerContext>,
) -> Result<Action, kube::runtime::finalizer::Error<kube::Error>> {
    kube::runtime::finalizer(
        &context.as_ref().challenge_api,
        "challenge_server",
        challenge,
        |event: Event<Challenge>| match event {
            Event::Apply(c) => Either::Left(apply_challenge(c, context.as_ref())),
            Event::Cleanup(c) => Either::Right(cleanup_challenge(c, context.as_ref())),
        },
    )
    .await
}

async fn apply_challenge(
    challenge: Arc<Challenge>,
    ctx: &ChallengerControllerContext,
) -> Result<Action, kube::Error> {
    ctx.last_seen_challenge
        .insert(Observed {
...
async fn cleanup_challenge(
    _challenge: Arc<Challenge>,
    _ctx: &ChallengerControllerContext,
) -> Result<Action, kube::Error> {
    Ok(Action::await_change())
}
...
fn error_policy(
    _challenge: Arc<Challenge>,
    e: &kube::runtime::finalizer::Error<kube::Error>,
    _context: Arc<ChallengerControllerContext>,
) -> Action {
    eprintln!("{:?}", e);
