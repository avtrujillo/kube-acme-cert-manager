...
pub async fn main() {
    ...
    let challenge_socket = SocketAddr::V4(SocketAddrV4::new(
    ...
    let last_seen_challenge: &'static LastSeen<Option<Arc<Challenge>>> =
        Box::leak(Box::new(LastSeen::new(None)));
    ...
    .await
    {
        ...
        Err(e) => {
            eprintln!(
                "Restarting challenge controller + server loop due to an error: {:?}",
                e
            );
        }
    }
...
pub async fn run_challenge_controller_and_server(
    ...
    let service_api: &'static Api<Service> =
        Box::leak(Box::new(Api::namespaced(kube_client.clone(), &namespace)));
    ...
    let server_fut = server::run_challenge_server(challenge_server_state, socket_addr);
    ...
    .map_err(|e| ChallengeServerWithControllerError::Controller(e));

    let server_handle = tokio::spawn(server_fut);

    let controller_handle = tokio::spawn(controller_fut);

    let join_result = futures::future::try_join(server_handle, controller_handle).await?;
    match join_result {
        (Ok(()), Ok(())) => Ok(()),
        (Err(e), Ok(())) => Err(e),
        (Ok(()), Err(e)) => Err(e),
...
impl<T: Clone + Sync> LastSeen<T> {
    pub fn new(inner: T) -> Self {
        Self(Mutex::new(Cell::new(Observed {
            inner,
    ...
    pub async fn insert(&self, observed: Observed<T>) {
        ...
        if observed_mutex
            .get_mut()
            .timestamp
            .duration_since(observed.timestamp)
            .is_zero()
        {
