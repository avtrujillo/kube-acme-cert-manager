...
use super::{ChallengeServerWithControllerError, LastSeen};
...
pub async fn run_challenge_server(
    state: ChallengeServerState,
    socket_addr: SocketAddr,
) -> Result<(), ChallengeServerWithControllerError> {
    let domain_str =
        std::env::var("DOMAIN").map_err(|e| ChallengeServerWithControllerError::DomainEnvVar(e))?;
    let domain = http::uri::Authority::try_from(domain_str)
        .map_err(|e| ChallengeServerWithControllerError::InvalidDomain(e))?;
    let listener = TcpListener::bind(socket_addr)
        .await
        .map_err(|e| ChallengeServerWithControllerError::Server(crate::AcmeServerError::Bind(e)))?;
    ...
    let router = Router::new()
        ...
        .fallback(async |http_uri: Uri| {
            match Uri::builder()
                .authority((http_uri.authority()).cloned().unwrap_or(domain))
                .path_and_query(
                    http_uri
                        .path_and_query()
                        .cloned()
                ...
                .build()
            {
                ...
                Ok(https_uri) => {
                    axum::response::Redirect::to(https_uri.to_string().as_str()).into_response()
                }
            }
        });
    ...
    axum::serve(listener, router)
        .await
        .map_err(|e| ChallengeServerWithControllerError::Server(crate::AcmeServerError::Axum(e)))
}
...
async fn challenge_handler(
    ...
    let challenge_req_uri = Uri::builder()
        .authority(format!("{}:{}", service_ip, 8089))
        .scheme("http")
        .path_and_query(challenge_path)
        .build()
        .map_err(|e| {
            eprintln!("{:?}", e);
    ...
    let challenge_req = state
        .reqwest_client
        .request(http::Method::GET, challenge_req_uri.to_string())
...
async fn get_service_list(
    ...
    let service_list = service_api
        .list(&ListParams {
            label_selector: Some(SERVICE_LABEL_SELECTOR.to_string()),
            ..Default::default()
        })
        .await
        .map_err(|e| {
